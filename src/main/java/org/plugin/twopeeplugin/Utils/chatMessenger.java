...
public class chatMessenger {
    ...
    public static void sendDiscordLink(Player player) {
        TextComponent inviteToWorld = new TextComponent(ChatColor.YELLOW + "Join Our Discord! Click HERE to Redirect..");
    ...
    public static void sendVerifiable(Player player) {
        player.sendMessage(ChatColor.YELLOW + "Verify the Course to Publish the Map! /verifycancel to Cancel");
        playLevelUpSound(player);
    }
    ...
    public static void sendGiveItem(Player player, String itemname) {
        player.sendMessage(ChatColor.YELLOW + "You got" + itemname + "!");
        playLevelUpSound(player);
    }
